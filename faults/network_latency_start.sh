...
function generate_assert_fault_running_command {
    # $1    -   Interface
    echo "tc q | grep netem | grep $1 | grep delay"
}

while [[ "$#" -gt 0 ]]; do
    case $1 in
        --delay-milliseconds) DELAYMILLISECONDS="$2"; shift ;;
        --jitter-milliseconds) JITTERMILLISECONDS="$2"; shift ;;
...
assert_argument "$DELAYMILLISECONDS" "--delay-milliseconds";
assert_argument "$JITTERMILLISECONDS" "--jitter-milliseconds";
